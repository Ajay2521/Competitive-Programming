...
// Space Complexity S = O(1), BruteForce run in constant space complexity
...
class Solution {
    public int[] twoSum(int[] nums, int target) {
        ...
        // looping through all the values in the array from "0" to "n-2"
        for(int i = 0; i<(nums.length-1); i++){
            
            // looping through all the values in the array from "1" to "n-1"
